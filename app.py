...
app = Flask(__name__)
app.secret_key = 'gB7@!$eR1z9_TravelSecret#42'
...
app.config['MAIL_USERNAME'] = 'gau879731raj@gmail.com'
app.config['MAIL_PASSWORD'] = 'bnwl wmja ghul shwh'
mail = Mail(app)
...
def login():
    ...
    if user.get('is_first_login', True):
        session['welcome_status'] = f"Welcome, {user['name']}!"
        users.update_one({'email': email}, {'$set': {'is_first_login': False}})
    else:
        session['welcome_status'] = f"Welcome back, {user['name']}!"
